...
│   └── vllm.yaml
├── cloud_providers/
│   ├── aws.yaml               ← AWS region defaults, SigV4 config
│   ├── azure.yaml
│   └── gcp.yaml
└── environments/
...
**`config/providers/bedrock.yaml`** — AWS-specific
```yaml
provider_name: bedrock
provider_type: aws_sdk
implementation_class: app.providers.bedrock_provider.BedrockProvider
...
│   ├── responses.py                 ← ChatResponse, EmbedResponse, HealthStatus
...
2. **Create Class**: `app/providers/mistral_provider.py` — extend `BaseProvider`, implement abstract methods
