...
What this service adds beyond CRUD:
    ...
    - Invalidates cached authorization grants after deployment changes.
...
class TenantDeploymentService:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize with persistence, authorization, secrets, and cache coherence."""
    ...
    async def create_deployment(
        ...
        The method validates foreign references first, stores the submitted
        credential in the secret backend, writes the record, then invalidates
        caches so the new deployment is routable immediately.
        """
        await self._access.ensure_tenant_admin(tenant_id, current_user)
        references = await self._references.ensure_deployment_create_references(
            tenant_id=tenant_id,
        ...
        secret_reference = await encode_credential(
            request.credential,
            build_credential_path("tenant-deployments", str(tenant_id), request.deployment_key),
            str(tenant_id),
            self._credential_writer,
            references.provider_auth_mode,
        )
    ...
    async def count_deployments(
        ...
        """Count deployments after the same authorization as the list query."""
        await self._access.ensure_tenant_read(tenant_id, current_user)
    ...
    async def _invalidate(self, tenant_id: UUID, deployment_key: str) -> None:
        """Invalidate authorization grants affected by deployment changes.

        Routing configuration is read directly from PostgreSQL, so there is no
        second deployment-config cache to invalidate.
        """
