...
_DEFAULT_PROVIDER_CONFIG = _ProviderBreakerConfig(
...
class _CircuitBreakerListener(aiobreaker.CircuitBreakerListener):
    ...
    def state_change(self, cb: aiobreaker.CircuitBreaker, old_state: aiobreaker.CircuitBreakerState, new_state: aiobreaker.CircuitBreakerState):
        logger.warning(
...
def _build_provider_storage(provider_name: str, cache: RedisCache) -> aiobreaker.storage.base.CircuitBreakerStorage:
...
def _create_provider_circuit_breaker(provider_name: str, cache: RedisCache) -> aiobreaker.CircuitBreaker:
...
async def get_provider_circuit_breaker(provider_name: str, cache: RedisCache) -> aiobreaker.CircuitBreaker:
